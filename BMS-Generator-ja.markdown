...
配置後、適格な Tap ノートがロングノート(LN)へプロモーション(昇格)される。Tap は、そのトークンのサンプル duration が `LN_MIN_DURATION_MS = 350`(人間の付随的な LN hold の median; 800 から引き下げた再評価は §8.8)以上のとき LN 候補となる。描画される hold 長は `LN_MAX_HOLD_TICKS = 96`(2 拍の可視 cap、v11)で cap され、長いサンプルが画面を埋め尽くすバーを描かないようにする。*オーディオ* のサンプルは最後まで再生される — 可視のバーだけが cap される。カウントは2つの governor で制限される: チャート全体の `LN_MAX_RATIO = 2%` cap と、予算が1つの領域へ山積みしないようにする per-窓の局所 cap(§8.8)。hold は、宣言されている場合は譜面の `#LNOBJ` トークンを用いて書き込まれる。

このゲートは長らく 800 ms(人間の LN duration のおおよそ p75)に固定され、*本来短い* 人間の LN の約 75% を遮断し、LN の多い曲を飢えさせた。§8.8 の再評価が2つを正した: corpus の付随 hold median は 353 ms であり、「一律に下げるとすべてが over-LN になる」という以前の懸念自体が誤りだった — 2% の比率 cap がすでにカウントを上限しているため、真の欠陥は *局所的* な山積み(ある譜面は LN の 57% を単一の8小節窓に置いた)であり、ゲートではなく per-窓 cap で修正された。
...
ロングノート(hold)は、スプリットルーターが表現しない6つ目のcharacterであり、専用の **opt-in** パスが扱う — DPのholdは疎な装飾(人間のDP corpusの18.4%にのみ存在、p50比率1.2%)であるため、`--dp --ln` を渡さない限りこの機能はオフである。このパスはDP分岐の最後尾で走り、すでに配置されたタップを **holdへ変換するだけ** である。タイミング・レーン・`(measure, idx, token)` マルチセットはすべて構成上保存される(ゲート失敗時はノートを削除するのではなくholdを *拒否* する)。

**ミラー優先.** holdの一次ソースは譜面ソースそのものである: SPソースの実在するキーチャンネルLNが、同じonsetのDPタップをholdへ昇格させる。(`#LNOBJ` エンコーディングは各レーンの最後のタップをzero-length Longとしてパースするため、duration > 0 のフィルタが必須 — さもなくば有病率が25.3%に膨らむ。)長いholdを先に、4%の比率cap(corpus p90)のもとで取ることで、音楽的に支配的なholdがcapを生き残る。

**800 msの床ではなくBPM認識フォールバック.** ソースがhold貧困なとき、フォールバックが長い音のトークンからcorpus median(1.2%)まで補填する。ここでのゲートは固定ミリ秒の床では *ない*: DP corpusのhold長はp50 = 365 ms(p25 = 238)であり、800 msの床は実在するDP holdの74%を排除する。代わりにトークンは、ローカルBPMで **拍量子化された** holdを維持できれば適格となる(240 ms prefilter ≈ corpus p25)。SP自身のLNゲートも同じ理由で同時に再評価され、800 → 350 msへ引き下げられた(SPの付随hold median 353 ms; 800 msの床は短い音の譜面のholdを完全に飢えさせた — ある密な譜面は可能な76 holdからゼロへ落ちた)。

**幾何.** holdはその周囲をゲートする(同じ手のみ、13,668-LNのcorpus調査より): ホストレーンと同側スクラッチはhold全体でhard禁止、ミドル隣接タップはsoft制限、自由な手は意図的に放置(holdの下でストリームが60%継続する)。ある規則は統計ではなく物理である — **1本の指が担当する2ボタンの一方を押している間、他方は押せない。** 薬指がボタン {2,3}、人差し指が {5,6} を担当するため、いずれかのボタンのholdは、hold中ずっとその相方ボタンのノートを禁止する。ペア集合 {% raw %}{{2,3},{5,6}}{% endraw %} はミラー対称(`S 1234567 │ 1234567 S` 配置で 2↔6, 3↔5)なので、同じraw-キー規則が両手に適用される。

**局所的な山積み防止.** チャート全体の比率capは *グローバル* な予算であり、放置すると長いトークンが最も多い領域へダンプされる(測定されたある譜面はholdの57%を単一の8小節窓に、別の譜面は全てを後半に集めた)。per-窓cap — 各8小節窓はグローバルLN比率の倍数までhold可能 — が予算を譜面全体へ広げつつ自然なクラスタリングは許容し、その数値を23%と譜面全体への分散へ下げる。capは絶対数ではなくローカルノートの *比率* である: corpusは窓あたり70+ holdを持つLN専用譜面に汚染されており、絶対アンカーは使えない。同じ窓capがSPパスも支配する。

### 8.9 Conformanceと DP固有の制限
...
- **6つのcharacterのうち1つが未カバー.** スプリットルーターはstream / peak / chordを扱い、ロングノートは専用パス(§8.8)を持つ。**soft** のcharacterのみまだ扱われていない。
- **戦略のauto-routingは未解決**(DR-DP7)。characterはユーザーが選ばなければならない。
- **normativeへの昇格は** より広範な検証、そしてより厳密なrail-weightのキャリブレーション(拒否されたノートからのスピルオーバーにより、現在のrailバイアスは正確な比率ではなく「feel(感覚)」である)を待っている。
...
`build_pool_universe`とラベリングパイプラインはどちらもLong eventを`(start, token)`のペアに平坦化し、hold長を捨てる — そのためML学習データにLN-スタイルの次元がなく、LN-awareな*モデル*は依然としてラベリングパイプラインの変更を要する。しかしRB経路はもはやblindではない: SPゲートはいまやcorpusアンカーされた350 ms + per-窓cap(§4.8)であり、DP-LNパス(§8.8)はソースの実在するLNをパース済みのeventsから直接読み取り(プールの平坦化を回避して)ミラーし、hold貧困なソースにはBPM認識フォールバックを置く。未解決なのは*モデル*側と、より広範なパッケージごとのLN-スタイル統計である。
...
LN_LOCAL_RATIO                 = 0.08   # per-窓 LN cap (予算を分散)
